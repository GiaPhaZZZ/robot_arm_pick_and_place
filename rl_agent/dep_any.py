...
class DepthAnythingDetectorNode(Node):
    ...
    def __init__(self):
        ...
        # Raised from 0.013 to 0.045 to avoid rejecting larger objects like the cube
        ...
        self.declare_parameter('hybrid_depth_pass',    True)
    ...
    def _is_object_black(self, cv_bgr: np.ndarray, x1: int, y1: int, x2: int, y2: int) -> bool:
        """
        Analyzes the HSV properties of the object to see if it's truly black
        versus a standard ambient lighting shadow on a workspace surface.
        """
        ...
        v_median = np.percentile(v_channel, 35)
        s_median = np.percentile(s_channel, 35)
        
        is_dark = v_median < 45.0
        is_neutral = s_median < 50.0
        
        return is_dark and is_neutral
    ...
    def _detect_depth_fallback(self, cv_bgr: np.ndarray, norm_depth: np.ndarray, h: int, w: int,
        ...
        for cnt in contours:
            ...
            # ── FIXED: Reject objects taking up almost the whole horizontal view field ──
            if bw2 > (w * 0.85):
                continue
